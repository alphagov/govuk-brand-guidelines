...
![Abstract shapes, including circles, stars, and rectangles, with a right-pointing arrow showing their transformation into completed documents and webpages.](./principles-flat-colour-geometric-shapes.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A wallet with bank cards, alongside editable pound-sign graphics and a delete icon, representing that maybe you won't need the pound sign as well as the bank cards.](./principles-low-detail.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A vector graphics pen tool editing the curved edge of a garage illustration containing a car.](./principles-corners.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A mouse cursor selects and adjusts a bounding box around an illustration of a person, positioning them next to another figure holding a potted plant.](./principles-layering.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![An illustration of a paper document with a paperclip, overlaid with a circular color wheel and an eyedropper tool selecting a color.](./principles-colour-palette.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A paint palette icon overlaid on a grid of various color swatches, representing color configuration options.](./principles-colour-tone.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A mobile phone mock up showing a numbered list of steps 2 to 4 of a process: enter address, continue on bank app or website to sign in, and bank confirms identity securely.](./principles-single-multiple.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A woman in a wheelchair holding a box with 2 other boxes stacked on the floor in front of her.](./principles-character-led.svg)
{% endgridCell%}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A finger interacts with a web form mockup displayed alongside a mobile phone version of the same interface.](./principles-information-led.svg)
{% endgridCell%}

{% endgrid %}

{% sectionHighlight { classes: 'app-section-highlight--light-blue govuk-!-margin-0' } %}
{% grid { columns: { tablet: 2 }, classes: 'govuk-!-margin-0' } %}
...
{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A collage of different illustrations of UK- centric details such as road signs, a red letter box, an umbrella, a football, cricket bat and ball, a poppy, and a daffodil.](./principles-relateable-details.svg)
{% endgridCell %}
