---
order: 2
title: Guide for creating illustrations
---
...
{% grid { columns: { desktop: 2 }, gap: '15px', classes: 'govuk-!-margin-0' } %}

{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A mechanic works under the bonnet of a car outside a garage, while another car is raised on a lift inside the workshop.](./creating-01-gather-research-1.png)
{% endgridCell %}

{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A mechanic working under the bonnet of a car outside a garage, with yellow boxes highlighting the mechanic, the garage sign, a car raised on a lift inside the workshop, and the workshop entrance.](./creating-01-gather-research-2.png)
{% endgridCell %}
...
![A rough sketch of a woman under the bonnet of a car outside a garage, a car raised on a lift inside the workshop and an MOT sign by the entrance.](creating-02-sketch-concept.png)
...
![Illustration of one car on a lift, and another viewed from the front with an MOT symbol above.](./creating-03-simplify-forms.svg)
...
![Various cursors editing a series of blue squares and a simplified human figure, representing the process of building and arranging elements within a digital scene.](./creating-04-composition.svg)
...
![A collage showing a stylised human figure being assembled from separate shapes, alongside two women with different skin tones shaking hands.](./creating-05-characters.svg)
...
![A mechanic works under the bonnet of a car outside an MOT garage, while another car is raised on a lift inside the workshop.](./creating-06-hierarchy.svg)

### 7. Consider colour

When creating a composition, use a refined selection of 4 to 6 colours rather than the full colour palette. Please refer to the [colour section](../../colour/) of the brand guidelines for more information.
...
{% grid { columns: { desktop: 3 }, gap: '15px' } %}

{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A wide range of colours along with their associated tints.](./creating-07-colour-1.svg)
{% endgridCell %}

{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A colour wheel shown with a dropper.](./creating-07-colour-2.svg)
{% endgridCell %}

{% gridCell { classes: 'app-grid__cell--image-full-width' } %}
![A limited colour palette, consisting of 6 colours.](./creating-07-colour-3.svg)
{% endgridCell %}
...
![An illustration of a female mechanic working under the bonnet of a car outside an MOT garage, while another car is raised on a lift inside the workshop.](./creating-08-consistency.svg)
