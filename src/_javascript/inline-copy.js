...
export class InlineCopy extends ConfigurableComponent {
  ...
  constructor($root, config) {
    ...
    // Pages can have a lot (hundreds!) of copy buttons on them. Instead of
    // having a screen reader announcement element for each one like on the
    // Design System website, they all share one status element.
    this.$status = document.querySelector('[data-inline-copy-status]')

    // If there is no status element, create one
    if (!this.$status) {
      const $status = document.createElement('div')
      $status.className = 'govuk-visually-hidden'
      $status.setAttribute('aria-live', 'assertive')
      $status.setAttribute('data-inline-copy-status', '')
  ...
  resetButton() {
    this.$button.innerHTML = copyButtonHtml(this.initialText)
  ...
  activeButton() {
    this.$button.innerHTML = copiedButtonHtml(this.initialText)
